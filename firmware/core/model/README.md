...
`AircraftObs` carries its `events::Stamp` rather than a pair of loose time fields, so the instant a position was heard is the same shape everywhere in the tree. `at_ms` beside it is a different fact: the instant the position was *true*, on `ports::Clock`, which is what the relative geometry aligns on.
