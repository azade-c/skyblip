...
Six dials over the GNSS-derived own-ship state, each with its own number: ground speed, subscale, altimeter, turn coordinator, track, vertical speed. The snapshot arrives in knots, feet and feet per minute, because the bank geometry is worked in them.

The top middle used to be an attitude dial pitching with the flight-path angle. It was a picture of two quantities the device cannot measure: there is no pitch sensor and no bank sensor, only a track rate and a climb rate, and an attitude a pilot could check against the window is the last place to show a number that was inferred. It is a subscale now, which is a quantity two sensors really do produce between them.

`QNH HPA` is the altimeter setting that makes this barometer read the height above mean sea level GNSS reports: `flight::qnh_from_alt`, derived once per baro sample in `OwnshipService` and smoothed over about half a minute, because weather moves the sea-level pressure by a hectopascal an hour and a GNSS altitude wanders that much in a minute. The needle is that derived value, the longer index mark is `state.qnh_pa`, the subscale the pilot has actually dialled in on the settings page, and the gap between them is how far the altimeter is from the air it is flying in. Nothing is applied automatically: a derived value that quietly became the setting would hide the one thing the dial is for.
