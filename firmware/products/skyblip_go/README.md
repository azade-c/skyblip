...
The fourth is the cell, and it is the only one that says something happened rather than naming a state. `SWITCHED OFF`, `FLAT BATTERY`, `PLUG IN TO WAKE`, stacked halfway between the mark and the bottom of the glass. All three lines earn their place: the device turned itself off, which a pilot who did not press anything has to be told; the reason, because the alternative guesses are a crash and a dead device; and the way back, because a low-battery shutdown is the one that withholds the wake pin, so the button does nothing and only a cable brings the device up (`core/power/shutdown.h`).
