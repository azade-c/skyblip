...
| `Unsupported` | the frame framed and carried a message type this firmware does not implement: ALP-TAS reads its 2024 position frame and nothing else, so Air V6 (type 0) and text messages (type 3) land here, and ADS-L reads §F.2.1's Traffic and nothing else, so Status (3), Remote Identification (6) and OGN Diagnostics (66) land here too |
