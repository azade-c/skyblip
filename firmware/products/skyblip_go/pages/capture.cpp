...
void row(ui::Canvas& fb, int line, char* buf, int n) {
    buf[n] = 0;
    fb.draw_text(kLeft, kTopY + line * kLineH, buf, true, 1);
}
...
void draw_price(ui::Canvas& fb, const bus::CaptureState& c) {
    char buf[40];
    int n = fmt_word(buf, "TAKES");
    n += fmt_uint(buf + n, c.price_sectors);
    n += fmt_string(buf + n, " OF ");
    n += fmt_uint(buf + n, c.pool_sectors);
    n += fmt_string(buf + n, " SECTORS");
    row(fb, 1, buf, n);

    n = fmt_word(buf, "EVICTS");
    n += fmt_uint(buf + n, c.price_flights);
...
void draw_faults(ui::Canvas& fb, const bus::CaptureState& c) {
...
void draw_capture(ui::Canvas& fb, const CaptureSnapshot& s) {
    ...
    n += fmt_string(buf + n, s.capture.armed ? "ARMED" : stopped_word(s.capture.stopped));
    row(fb, 0, buf, n);

    if (s.capture.armed)
        draw_progress(fb, s.capture);
    else
        draw_price(fb, s.capture);

    if (!s.capture.armed && s.capture.records > 0) {
        n = fmt_word(buf, "WROTE");
        n += fmt_uint(buf + n, s.capture.records);
        n += fmt_string(buf + n, " DROP ");
        n += fmt_uint(buf + n, s.capture.dropped);
        row(fb, 4, buf, n);
    }

    draw_faults(fb, s.capture);
    draw_hint(fb, s);
}
