...
`TAXI` is a display state and nothing else: `ScreenService::taxiing` is a fix, the ground state `core/flight/state.h` decided, and a ground speed at or above `flight::kGroundSpeedQ`, 2.5 m/s. That is the same threshold the state machine calls a standstill, so the word begins exactly where GROUND's definition of still ends and there is no third opinion about it. The device transmits, logs and locks the same way it does parked, because ADS-L G.1.4 has two states and taxiing is one of them. The band runs all the way to 15 m/s, which is where a takeoff is declared: nobody taxis at 30 kt, and every takeoff roll passes it.
