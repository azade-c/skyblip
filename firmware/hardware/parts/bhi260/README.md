...
`GYRO` (virtual sensor 13, the bias-corrected gyroscope) is configured beside it when the image carries it, at the same rate, range 500 deg/s - 0.015 deg/s a count, against the 0.7 deg/s the GNSS track rate can express, and past what a spin would reach on any axis the instrument cares about. It is the one sensor here with a power bill worth measuring: a gyroscope runs the better part of a milliamp where the accelerometer runs tens of microamps, so a unit that never draws the turn coordinator should not be enabling it. Two things decide, and both have to say yes. The bitmap read at `Configuring` is the first: no gyroscope in the image, no configuration sent, and `core/flight/rate` falls back to the track rate on its own. The second is `request_gyroscope()`, which is the product handing over the pilot's `GYRO` row (`products/skyblip_go/pages/README.md`) and is false until somebody asks, so nothing enables this sensor by omission.

The request is followed from `Running` as well as at `Configuring`, because a pilot changes it in flight and a sensor hub is not a device that needs rebooting to stop a sensor. A sample rate of zero is the hub's unsubscribe - `stop_sensor()`, the same command that configures one - and with nothing left asking for the gyroscope the hub powers the physical part down itself. Either direction takes effect on the next `service()` pass, at most `kSamplePeriodMs` away; what a pilot sees is the two seconds `core/flight/rate` and `core/flight/bank` take to go stale and hand the instruments back to the fix. Coming the other way the frames are immediate and the zero is not: `TurnRate` relearns its trim over about thirteen seconds, starting from the track rate it was already holding.
