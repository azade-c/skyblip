...
The instant is drawn by the same `mix(address ^ mix(utc))` every other burst is drawn by, over the width the window leaves once the air time and the completion slack are taken off it: 1000 to 1190, which is 191 instants, exactly as many as slot 1's own position window offers between 800 and 990. The two bursts cannot overlap, and no rule was added to keep them apart. The arithmetic already does it: a position burst completes by 995 because §C.5 ends the direct slot at 1000, and that is where the callsign window opens. What the 5 ms between them cannot always buy is the re-arm, which waits for the next service pass 10 ms later, so a position drawn at the very end of its window and a name drawn at the very start of its own costs that name and never a position. It is about one ident second in two thousand, and the next one is ten seconds later.
