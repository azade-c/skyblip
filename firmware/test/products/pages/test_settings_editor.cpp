...
bool row_label_reads(const Glass& fb, SettingsRow row, bool focused) {
    return reads_at(fb, kSettingsLeftX, settings_row_text_y(row), settings_row_label(row),
                    !focused);
}

bool row_value_reads(const Glass& fb, SettingsRow row, const char* value, bool focused) {
...
Glass page(const SettingsValues& values, SettingsRow focus) {
...
TEST_CASE("settings page: every row names what it holds, and the focused one is reversed out") {
    ...
    const Glass fb = page(values, SettingsRow::Volume);
    ...
    CHECK(kSettingsHintY + 7 < Glass::kH);
}
...
TEST_CASE("settings page: a category the phone stored but the page does not name is still shown") {
    ...
    const Glass fb = page(values, SettingsRow::Identity);
