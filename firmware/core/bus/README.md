...
|---|---|---|
| `panel_partial_refreshes` | frames the glass took on the partial waveform, about 460 ms each | one per `present()`, plus the black wipe a page swap costs, which the SSD1681 also drives partial |
| `panel_full_refreshes` | frames that took the full waveform, about 2.5 s each | one per `present()` asked for `Refresh::Full`, which on this product is the frame the glass wears parked |
