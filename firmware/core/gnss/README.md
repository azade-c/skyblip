...
A fix does not switch the levels off. A pilot holding the page open while the device transmits is watching the sky he asked for, and bars that stop the moment the fix lands are the page going blank at the instant it starts to matter. The bill is paid on the wire instead: RMC can arrive later in the second than the fix burst alone would put it, and `Transmitter::kFixLagMaxMs` is what refuses a burst that arrives too late to be the second it claims. Holding the page up through a flight can therefore cost direct slots, which is a price the page asks for by being open.
