...
The turn coordinator's title carries its unit, `TURN D/S`, because its dial shows a bank angle inferred from the track rate while its number is the track rate itself, and a number in degrees per second beside a picture of degrees needs saying which it is. The tenth of a degree per second the digit would suggest is not there to show: `core/flight/turn.h` differentiates a cordic9 track, 45/64 of a degree per unit, over a one-second window, so the smallest step the sensor can express is 0.7 deg/s.
