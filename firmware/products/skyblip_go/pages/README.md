...
`TAXI` is a display state and nothing else: `ScreenService::taxiing` is a fix, the ground state `core/flight/state.h` decided, and a ground speed of 2 m/s or more, which is `kLandingMotionE8` read as a speed. The device transmits, logs and locks the same way it does parked, because ADS-L G.1.4 has two states and taxiing is one of them. Its upper end belongs to the state machine rather than to this rule: a taxi held above the 4 m/s takeoff criterion for five seconds is declared airborne, so a brisk taxi reads `FLIGHT`, the same way OGN's criterion reads it.
