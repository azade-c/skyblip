...
// 30 kt is a speed nobody taxis at and every takeoff roll passes a second before it flies.
TEST_CASE("flight: a takeoff is the speed no taxi holds, and it waits for nothing") {
    ...
    CHECK(hold(monitor, 1, 8.0, 0.0) == FlightState::OnGround);   // a brisk taxi
    CHECK(hold(monitor, 1, 12.0, 0.0) == FlightState::OnGround);  // the roll, accelerating
    CHECK(hold(monitor, 1, 18.0, 0.0) == FlightState::Airborne);
}
...
TEST_CASE("flight: a landing is a standstill, and a fast rollout is not one yet") {
    ...
    CHECK(hold(monitor, 1, 2.0, 0.0) == FlightState::OnGround);
}
...
TEST_CASE("flight: a bounce during a rollout is a second takeoff and a second landing") {
    FlightMonitor monitor;
    REQUIRE(hold(monitor, 3, 25.0, 0.0) == FlightState::Airborne);
    REQUIRE(hold(monitor, 1, 1.0, 0.0) == FlightState::OnGround);

    CHECK(hold(monitor, 1, 10.0, 2.5) == FlightState::Airborne);
    CHECK(hold(monitor, 1, 1.0, 0.0) == FlightState::OnGround);
}
...
TEST_CASE("flight: a fix nobody should trust does not take off on its own") {
    ...
    CHECK(hold(monitor, 20, 16.0, 0.0, 3000) == FlightState::OnGround);

    // The same movement on a fix worth trusting is a takeoff.
    CHECK(hold(monitor, 1, 16.0, 0.0, 90) == FlightState::Airborne);
}
...
TEST_CASE("flight: the dilution of precision cannot land an aircraft") {
    CHECK_FALSE(flight_evidence(solution(16.0, 0.0, 3000)));
    CHECK_FALSE(ground_evidence(solution(16.0, 0.0, 3000)));
    CHECK(flight_evidence(solution(16.0, 0.0, 90)));

    CHECK(flight_evidence(solution(3.0, 2.0)));
    CHECK(flight_evidence(solution(3.0, -2.0)));
    CHECK_FALSE(flight_evidence(solution(0.0, 4.0)));
    CHECK(ground_evidence(solution(2.0, 0.5)));
    CHECK_FALSE(ground_evidence(solution(2.0, 1.5)));
    // A receiver that reports no DOP at all is not punished for it.
    CHECK(flight_evidence(solution(15.0, 0.0, 0)));
}
...
TEST_CASE("flight: a device switched on in the air says so at once") {
    ...
    CHECK(rolling.update(solution(8.0, 0.0)) == FlightState::OnGround);
