...
`gyro_enabled` is off on a new unit and off after a migration, and that is a decision rather than an oversight. A unit that stored version 5 stored no choice about the gyroscope: it ran because the firmware had no way not to run it, and a migration that invented `true` would be preserving an accident. So every unit on this firmware comes up with the milliamp unspent and a menu row saying where it went (`pages/README.md`). It is also the one setting accepted on "set" and absent from the "get" reply for a reason that is not a bench trim's: that reply has nine bytes of headroom and `"gyro":true` is twelve, and unlike a trim the device answers the question on its own glass. `Product::step` hands the wish to the board on every pass, which is what turns a menu press into a sensor that stops.
