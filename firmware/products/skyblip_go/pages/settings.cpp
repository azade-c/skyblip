...
void row_text(ui::Canvas& fb, SettingsRow row, const char* label, const char* value, bool focused) {
    const int top = settings_row_top(row);
    const int y = settings_row_text_y(row);
    if (focused) fb.rect(2, top, kGlassW - 4, kSettingsRowHeight - 1, true, /*fill=*/true);
...
void draw_settings(ui::Canvas& fb, const SettingsSnapshot& s) {
    fb.clear(true);
    fb.draw_text(kSettingsLeftX - 2, kHeaderY, "SETTINGS", true, 2);
    fb.hline(kSettingsLeftX - 2, kHeaderRuleY, kGlassW - 2 * (kSettingsLeftX - 2), true);
...
// go::validate the companion link's JSON goes through: a page cannot
