...
It inverts rather than fills. The threat, its leader line and its altitude tag are inside the sector it points at, and a filled wedge would erase the one aircraft it was drawn for; reversed out, they are the most legible things on the glass. Own ship is left alone - the wedge starts 15 px out - and so is the footer band, which the plot already keeps off: the clock, the range and the count are readings, and a reading a pilot has to invert in their head is a reading they get wrong. The state word in the ring is a reading too, and a quadrant reaches it where the old narrow slice never did, so it is drawn last, over the wedge rather than under it.
...
The bearing is the loudest target's, nearest first where two are level. There is one wedge because there is one instruction, and a page with two of them is a page that has told a pilot to look in two places at once.

What a flash from the first grade costs is a panel refresh a second for as long as anything is in the ring, which in a busy circuit is most of the flight, and that is exactly what the pad's long touch is for. `DISMISSED` stands in the ring where the state words stand, and it is the answer to why a device with an aircraft under alarm on its glass is quiet and still. The wedge stays and stops flashing: what the long touch took back is the noise and the motion, not the picture. Anything worse takes the silence back on the pass it happens, and the flashing comes back with it.
...
A hub that stopped names the stage it stopped in and why: `IMU BOOT VERIFY` is an image the part refused, `IMU LOAD DOWN` is a bus that quit mid-upload, `IMU HIF TIMEOUT` is a part that answered its address and never became ready. Without it the six-pack's empty cage is the only symptom, and an empty cage cannot say which of those happened. The field is 16 cells at its widest, which is what the row has left past `TRUE`.
