...
What this costs is the reason it is not always on. At 9600 baud the fix burst is 320 bytes, a third of the second; the widest GSV set the part can send, four talkers and thirty-odd satellites four to a sentence at about 72 bytes each, is another 648. Together that is `L76k::kSearchingBurstMs`, over a second, which means RMC would arrive after the ADS-L direct slot has opened and `Transmitter::kFixLagMaxMs` would refuse the burst. So GSV is asked for only while the `sats` page is on the glass and `timing::own_ship_transmits` is false, and given up the moment either goes, one `$PCAS03` each way with a null in every field but nGSV, which the receiver reads as keep what you were doing.

The page is the other half of that gate because it is the only thing on the device that reads a level. 648 bytes a second is 675 ms of line at 9600 baud, which is the receiver talking and the UART waking the core for two thirds of every second, for a picture of the sky nobody is looking at. A device searching on the apron with the radar up therefore asks for nothing beyond the fix burst, and what `status` and the ring say about the wait - `SILENT`, `BLIND`, `SOLVING` and the seconds on the rung - is read out of the sentences already on the wire. `bus::GnssStatus::levels_wanted` is where `ScreenService` says which page is up, and the board ANDs it with the transmit gate.

What the pilot pays for it is one second: GSV arrives on the solution after the `$PCAS03` lands, so the page opens on an empty sky and fills on the next second. That is the cost of a picture asked for by hand rather than one kept warm all flight.
